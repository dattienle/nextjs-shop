  ...
  if (!isValidArray(cartStore.list))
    return (
      <>
        ...
        <p>
          Hãy tham khảo các{" "}
          <Link className="text-indigo-600 underline" href={"/products"}>
            sản phẩm
          </Link>{" "}
          khác của chúng tôi
        </p>
              ...
              {cartStore.list.map((item: TCartItem, index: number) => (
                <TableRow key={item.variant_id}>
                  ...
                  <TableCell className="text-right">
                    {(
                      item.product_variant.variant_price * item.quantity
          ...
          <div className="flex flex-col gap-3 my-5 w-full items-end sticky top-28">
            <p>
              ...
              {cartStore.list
                .reduce(
                  (acc, item) =>
                    acc + item.product_variant.variant_price * item.quantity,
                  0
                )
                .toLocaleString("vi-VN")}
              đ
